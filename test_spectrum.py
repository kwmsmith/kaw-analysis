...
def test_laplacian():
    ...
    def laplacian_gen(N,m,n):
        ...
        rarr_laplacian = spect.laplacian(rarr)
        X,Y = np.ogrid[0:N, 0:N]
        X -= N/2; Y -= N/2
...
def test_cderivative():
    def cderiv_gen(N,m,n):
        rarr = sin_cos_arr(N,m,n)
        rarr_xx = spect.cderivative(rarr, 'X_DIR', order=2)
        rarr_yy = spect.cderivative(rarr, 'Y_DIR', order=2)
        rarr_laplacian = spect.laplacian(rarr)
...
def test_cderiv2():
    def sin_gen(N, m):
        rarr = sin_arr(N,m)
        rarr_x = spect.cderivative(rarr, 'X_DIR', order=1) * rarr.size
        rarr_y = spect.cderivative(rarr, 'Y_DIR', order=1) * rarr.size
